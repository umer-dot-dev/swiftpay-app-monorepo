    ...
    <q-header :class="['lt-md', $q.dark.isActive ? 'bg-dark text-white' : 'bg-grey-1 text-primary']" flat>
      <q-toolbar class="q-px-md q-py-sm">
        ...
        <q-btn round flat icon="notifications" class="q-ml-sm">
          <q-badge v-if="notificationStore.unreadCount > 0" floating color="red" rounded border>
    ...
    >
      <div class="q-pa-lg q-pt-xl">
        <div class="row items-center q-gutter-sm q-mb-xl q-px-sm">
          ...
          <span class="text-h5 text-weight-bolder tracking-tight" :class="$q.dark.isActive ? 'text-white' : 'text-dark'">SwiftPay<span class="text-primary">.</span></span>
        </div>

        <div class="text-caption text-grey-5 text-uppercase tracking-widest q-px-sm q-mb-md font-weight-bold">{{ $t('menu') }}</div>
      ...
      <div class="absolute-bottom q-pa-lg">
        ...
        <q-item clickable v-ripple @click="handleLogout" class="nav-item q-pa-md border-radius-16 text-grey-7 logout-item">
          ...
          <q-item-section>
            <q-item-label class="text-weight-bold nav-label">{{ $t('logout') }}</q-item-label>
          </q-item-section>
        </q-item>
      </div>
    </q-drawer>
...
const navigationLinks = computed(() => [
  ...
  { title: t('transactions'), icon: 'sync_alt', link: '/transactions' },
  { title: t('wallet'), icon: 'account_balance_wallet', link: '/wallet' },
  { title: t('cards'), icon: 'style', link: '/cards' },
  { title: t('settings'), icon: 'tune', link: '/settings' },
]);
</script>
...
body.body--dark .active-nav-item {
  background: rgba(37, 99, 235, 0.2);
  color: #fff !important;
}

body.body--dark .nav-item {
  color: #94a3b8;
}
