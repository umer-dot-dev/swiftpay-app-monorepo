  ...
  <q-menu anchor="bottom right" self="top right" :offset="[0, 8]" class="notification-menu border-radius-24 no-shadow">
    <div class="row items-center justify-between q-pa-md border-bottom">
      ...
      <div class="row items-center q-gutter-x-sm">
        <q-btn flat dense no-caps color="primary" label="Mark all read" class="text-caption text-weight-medium" @click.stop.prevent="store.markAllAsRead" v-if="store.notifications.length > 0" />
        <q-btn flat round dense color="negative" icon="delete" size="sm" @click.stop.prevent="confirmDeleteAll" v-if="store.notifications.length > 0">
...
const confirmDeleteAll = () => {
  $q.dialog({
    ...
    void store.clearAllNotifications()
  })
}
